 ...
 * 阴影所有的在给定的时间播放的所有音符
 */
public class MusicBook extends SurfaceView implements SurfaceHolder.Callback,
        ScrollAnimationListener, MidiPlayer.MidiPlayerCallback, Runnable {
     ...
     * The left margin
     */
    public static final int LineSpace = 14;// 在乐谱中两条线之间的空隙
    /**
...
//        }
    ...
    private void drawToBuffer(int left, int top) {
        Log.d("sss", "------------>drawToBuffer:letf=" + left + ", top=" + top);
        if (staffs == null) {
            return;
        }
        bufferX = left;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        boolean result = false;//scrollAnimation.onTouchEvent(event);
        switch (action) {
    ...
    public void surfaceCreated(SurfaceHolder surfaceHolder) {
        Log.d("sss", "surfaceview--------->surfaceCreated");
        surfaceReady = true;
//        new Thread(this).start();
    ...
    public void surfaceChanged(SurfaceHolder surfaceHolder, int i, int i1, int i2) {
        Log.d("sss", "surfaceview--------->surfaceChanged");
        callOnDraw();
    }

    @Override
    public void surfaceDestroyed(SurfaceHolder surfaceHolder) {
        Log.d("sss", "surfaceview--------->surfaceDestroyed");
        surfaceReady = false;
    }
